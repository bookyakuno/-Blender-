...
アニメーションの動きだけを確認したい時にいいかも。
注意：Escでの再生解除には対応していないので、アニメーションをストップする場合は同じショートカットで終了させて下さい。でないと"レンダリングする物のみ"のままになっていしまいます。


アドオンをオンにし、以下をframe内に手動でショートカット登録してください
